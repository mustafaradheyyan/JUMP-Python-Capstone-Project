...
URL = "https://mfcp436giirb7jwlvgizni2w7m0agrmc.lambda-url.us-west-1.on.aws/csv"
...
def get_payload(payload_df: pd.DataFrame):
    i = 0
    num = 1
...
def send_payload(payloads: types.GeneratorType, session: requests.Session):
    for file_name, payload_df in payloads:
...
def main():
    ...
    payload_df_generator = get_payload(earthquake_df)
    
    send_payload(payload_df_generator, session)
